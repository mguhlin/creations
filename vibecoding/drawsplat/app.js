...
function setTool(next){tool=next; document.querySelectorAll('#toolButtons button').forEach(b=>b.classList.toggle('active',b.dataset.tool===tool)); if(tool!=='connector') connectorPendingFrom=null; applyToolContext(); syncSimpleColor()}
...
gid('removeBgColorBtn')?.addEventListener('click',async()=>{const p=panel(); if(!p.bgImage){setStatus('Set a background image first.','danger'); return} try{const corner=await sampleCornerColor(p.bgImage); const inp=gid('removeBgColorPicker'); if(inp){inp.value=corner; inp.click()}}catch(err){setStatus('Could not read image. '+err.message,'danger')}});
gid('simpleRemoveBgColorBtn')?.addEventListener('click',()=>gid('removeBgColorBtn').click());
gid('removeBgColorPicker')?.addEventListener('change',async e=>{const target=e.target.value; const p=panel(); if(!p.bgImage) return; setStatus('Removing color…','success'); try{const newUrl=await removeColorFromImage(p.bgImage,target,40); p.bgImage=newUrl; render(); saveState(); setStatus('Background color removed.','success')}catch(err){setStatus('Failed to remove color. '+err.message,'danger')}});
