...
gid('removeBgColorBtn')?.addEventListener('click',async()=>{const p=panel(); if(!p.bgImage){setStatus('Set a background image first.','danger'); return} try{const corner=await sampleCornerColor(p.bgImage); gid('bgRemoveColorInput').value=corner; const tolEl=gid('bgRemoveTolerance'); if(tolEl) gid('bgRemoveToleranceValue').textContent=tolEl.value; gid('bgRemoveDialog').showModal()}catch(err){setStatus('Could not read image. '+err.message,'danger')}});
